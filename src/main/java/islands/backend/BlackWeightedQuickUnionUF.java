 ...
 *  @author Kevin Wayne
 */
public class BlackWeightedQuickUnionUF {
    public static final int EMPTY = -1;
    private final int[] parent; // parent[i] = parent of i
    private final int[] size; // size[i] = number of elements in subtree rooted at i
    private int islands; // number of components
    ArrayList<Integer> left = new ArrayList<>(); // to hold the parents of tiles in left column
    ArrayList<Integer> right = new ArrayList<>(); // to hold the parents of tiles in right column
     ...
     */
    public BlackWeightedQuickUnionUF(int n) {
        islands = 0;
        parent = new int[n];
        size = new int[n];
        Arrays.fill(size, 1);
    ...
    public boolean gameOver() {
        if (left.size() > 0 && right.size() > 0) {
            for (Integer n: left) {
                if (right.contains(n)) return true;
            }
        }
        return false;
    }
    ...
    public void processTile(int row, int col, int sz) {
        ...
        // if tile is in left or right column, this adds parent of the tile after processing to corresponding ArrayList.
        if (col == 0 && !left.contains(parent[currentTile])) left.add(parent[currentTile]);
        if (col == sz-1&& !right.contains(parent[currentTile])) right.add(parent[currentTile]);
    }
}
